...
let __my__ = [...firstArray, ...secondArray]; 
/**
...
let __my2__ = function() {
    let ok = [];
    for (let i = fourthArray.length-1; i > -1; i--) {
        ok.push(fourthArray[i])
    }
    return ok; 
}
/**
...
let __my3__ = [...new Set(fifthArray)];
/**
...
let __my4__ = sixthArray.sort().join()===seventhArray.sort().join();
// eighthArray and ninthArray are not
let eighthArray = [1, 2, true, false, 'hello', 3, 'moi moi'];
let ninthArray = [];
let __my5__ = !(eighthArray.sort().join()===ninthArray.sort().join()); //true (!)
/**
...
let __my6__ = tenthArray.flat(100);
/**
...
let __my7__ = peopleInfo.filter(ee=>/^[a-z]$.test(ee)/&&!ee.includes('@')&&!/\d+/.test(ee));
let __my8__ = peopleInfo.filter(el=>el.includes('@'));//k
let __my9__ = peopleInfo.join(' ').match(/\w+@\w+\.com/g);//k
/**
...
let __my10__ = sample.split(' ').map(e=>e.includes('o')?1:0)